...
/**
 * @brief Инициализация подсистемы GLFW и создание окна.
 * * Настраивает параметры контекста:
 * - OpenGL 4.5 Core Profile.
 * - Отключение изменения размера окна вручную (опционально).
 * - Настройка обратных вызовов (Callbacks) для системы событий.
 * * @note Включает V-Sync через glfwSwapInterval(1).
 */
void Application::InitGLFW() {
    ...
    /** @todo Перенести настройку параметров рендеринга (Depth Test, Face Culling)
     * в отдельный класс RenderCommand или RendererAPI.
    */
 ...
 * * Здесь реализуется логика переключения между режимом игры и режимом интерфейса:
 * - **L-ALT**: Освобождает курсор для работы с ImGui.
 * - **Left Click**: Захватывает курсор обратно в игру (если не кликнули по окну ImGui).
 * * @details Используется проверка ImGui::GetIO().WantCaptureMouse, чтобы клики по
 * кнопкам интерфейса не приводили к захвату курсора в игру.
 */
 ...
 * 2. Обновление Uniform-переменных шейдера (ViewProjection, Lighting).
 * 3. Вызов отрисовки мира (m_World->Render).
 * 4. Формирование и вывод отладочного интерфейса ImGui.
 * 5. Своп буферов и опрос системных событий.
 */
